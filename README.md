...
The whole thing is a single ~350-line Python script with no daemon and no background process. Run `./setup.sh` to install — X11 only; the hotkeys bind themselves on XFCE. On Wayland it needs a small change first — your agent will find the instructions in `CLAUDE.md`.
