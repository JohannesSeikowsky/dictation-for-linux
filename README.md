...
The whole thing is a single ~350-line Python script with no daemon and no background process. Run `./setup.sh` to install. 

ElevenLabs and an Anthropic API keys need to be added to `.env`. The estimated cost is roughly $0.09 a day for 20 minutes of dictation.
